...
class EGNN(nn.Module):
    ...
    def forward(self, feats, coors, edges=None, mask=None, adj_mat=None):
        ...
        m_ij = self.edge_mlp(edge_input.double())
...
class EGNN_Network(nn.Module):
    def __init__(self, *, depth, dim, num_tokens=None, num_edge_tokens=None, num_positions=None, edge_dim=0,
                 ...
                 global_linear_attn_dim_head=64, num_global_tokens=4, aggregate=True, **kwargs):
        super().__init__()
...
class predictor(nn.Module):
    def __init__(self, dim, dropout=0.1):
        ...
        self.out = nn.Sequential(nn.Linear(dim, dim), nn.ReLU(), nn.Dropout(p=dropout), nn.Linear(dim, 1))
